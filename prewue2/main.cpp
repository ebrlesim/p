...
int main()
{
    ...
    int stranka =0, stop =1, k=0 , start = 0; // proměné k == kolikátý řádek, stop kde je poslední řádek, start prvnířádek ale to tam teoreticky nebude muset býti 
    
    // otevření textového dokumentu v kterém budou jazykové podklady
    std :: ifstream vysledek;
    vysledek.open("test.txt");
    // vektor naplněný informacema které jsou v první vrstvě menu
    String h [15];
    std :: string line;
    ...
    {   
        ...
        if (k==i)
        {
            ...
            menu2.setString(h[i+1+(stranka*6)]);
            window.draw(menu2);
            }
        }else
        {
            ...
            menu.setString(h[i+1+(stranka*6)]);
            window.draw(menu);
            }
        }
